...
const Line = dynamic(() => import("../../../../../components/line-nivo"), {ssr: false});
...
export default function Tasks() {
    ...
    const ResponsiveGrid = ({reversed: bool = false, children}: any) => (
        <FlexGrid
        ...
        <FlexGridItem maxWidth={responsiveSideNavWidth}>
            <Navigation
                ...
                onChange={({event, item}) => {
                    ...
                    setVariant(fromVariantId(item.itemId))
                }}
                overrides={{
                    NavItemContainer: {
                        style: ({$theme}) => ({
            ...
            <ParagraphSmall color="gray">
                * If there are more than 100 normal data points, 100 random points are sampled for visualization. If
                there are more than 100 outliers, we sample an equal amount of normal points.
            </ParagraphSmall>
        </FlexGridItem>
    )
    ...
    const Content = (id: string, classes: number, fraction: number, dataset: Dataset) => (
        <React.Fragment>
            <ParagraphSmall $style={{textAlign: "center", padding: "7px 15px", border: "1px dashed"}}>
                <b>{id}</b> with {fraction * 100}% of outliers
                and {classes} normal {classes > 1 ? "classes" : "class"} *
            </ParagraphSmall>
            <Block
                maxWidth={responsiveSideNavWidth}
                display={"flex"}
                alignItems={"center"}
    ...
    return (
        <React.Fragment>
            <Block
                ...
                margin="auto"
            >
                {isLoading ? Loading : query.caseOf({
                    Just: query => data.caseOf({
                        Just: dataset => Content(query.id, query.classes, query.fraction, dataset),
                        Nothing: () => <p>Could not load task.</p>
                    }),
                    Nothing: () => <p>Invalid query.</p>
                })}
            </Block>
        </React.Fragment>
    );
